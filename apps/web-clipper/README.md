...
The clipper uses the same inline Tag model as the desktop editor. Enter a tag
name with or without `#`, then press Space, Enter, or comma to finish it.
Multiple tags are saved in the Markdown body as `#research #web/project`, not
in frontmatter. Unicode names, `_`, `-`, and `/` hierarchy are supported.

After the note is saved, the desktop Tags panel indexes these body hashtags
automatically. Search and graph filters omit the leading hash, for example
`tag:research`. Frontmatter properties named `tag` or `tags` are ignored and
are never generated by the clipper.

## Current Features

- Templates support variables such as `{{title}}`, `{{url}}`, `{{content}}`, `{{tags}}`, and `{{folder}}`. `{{tags}}` renders the selected values as space-separated inline hashtags.
