...
test("uses paired target-specific overrides without scheduling a build", () => {
    const plan = createCodexRuntimeBundlePlan({
        ...
        workspaceRoot,
        env: {
            BIFROSTWRITE_CODEX_ACP_BUNDLE_BIN_ARM64: "/inputs/codex-acp",
            BIFROSTWRITE_CODEX_CODE_MODE_HOST_BUNDLE_BIN_ARM64:
...
test("uses paired Windows overrides with executable names", () => {
    const plan = createCodexRuntimeBundlePlan({
        ...
        workspaceRoot,
        env: {
            BIFROSTWRITE_CODEX_ACP_BUNDLE_BIN_X64: "/inputs/codex-acp.exe",
            BIFROSTWRITE_CODEX_CODE_MODE_HOST_BUNDLE_BIN_X64:
...
test("uses paired generic overrides without scheduling a build", () => {
    const plan = createCodexRuntimeBundlePlan({
        ...
        workspaceRoot,
        env: {
            BIFROSTWRITE_CODEX_ACP_BUNDLE_BIN: "/inputs/codex-acp",
            BIFROSTWRITE_CODEX_CODE_MODE_HOST_BUNDLE_BIN:
                "/inputs/codex-code-mode-host",
        },
    });
        ...
        () =>
            createCodexRuntimeBundlePlan({
                ...
                env: {
                    ...
                    BIFROSTWRITE_CODEX_CODE_MODE_HOST_BUNDLE_BIN:
                        "/inputs/codex-code-mode-host",
                    BIFROSTWRITE_CODEX_ACP_BUNDLE_BIN_ARM64:
                        "/arm64/codex-acp",
                    BIFROSTWRITE_CODEX_CODE_MODE_HOST_BUNDLE_BIN_ARM64:
...
test("universal staging requires four inputs and produces two outputs", async () => {
    const env = {
        BIFROSTWRITE_CODEX_ACP_BUNDLE_BIN_ARM64: "/arm64/codex-acp",
        BIFROSTWRITE_CODEX_CODE_MODE_HOST_BUNDLE_BIN_ARM64:
            "/arm64/codex-code-mode-host",
        BIFROSTWRITE_CODEX_ACP_BUNDLE_BIN_X64: "/x64/codex-acp",
        BIFROSTWRITE_CODEX_CODE_MODE_HOST_BUNDLE_BIN_X64:
    ...
    const headers = new Map([
        [env.BIFROSTWRITE_CODEX_ACP_BUNDLE_BIN_ARM64, machoHeader(0x0100000c)],
        [env.BIFROSTWRITE_CODEX_CODE_MODE_HOST_BUNDLE_BIN_ARM64, machoHeader(0x0100000c)],
        [env.BIFROSTWRITE_CODEX_ACP_BUNDLE_BIN_X64, machoHeader(0x01000007)],
        [env.BIFROSTWRITE_CODEX_CODE_MODE_HOST_BUNDLE_BIN_X64, machoHeader(0x01000007)],
    ]);
...
test("universal staging rejects one missing binary input", async () => {
    ...
    await assert.rejects(
        validateCodexRuntimeBundleInputs(
            plan,
            existingPaths(
                env.BIFROSTWRITE_CODEX_ACP_BUNDLE_BIN_ARM64,
                env.BIFROSTWRITE_CODEX_CODE_MODE_HOST_BUNDLE_BIN_ARM64,
                env.BIFROSTWRITE_CODEX_ACP_BUNDLE_BIN_X64,
            ),
        ),
