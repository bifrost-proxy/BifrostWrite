...
} from "./stage-native-runtime-assets-helpers.mjs";
...
function resolveHostRustTarget() {
    ...
    throw new Error(
        `Unsupported host platform for Tauri sidecar staging: ${process.platform}/${process.arch}`,
    );
}
...
function embeddedNodeVersion() {
    return (
        process.env.BIFROSTWRITE_EMBEDDED_NODE_VERSION?.trim() ||
...
async function resolveEmbeddedNodeSource(targetTriple) {
    if (isMacUniversalTarget(targetTriple)) {
        const arm64Source =
            process.env.BIFROSTWRITE_EMBEDDED_NODE_BIN_ARM64?.trim()
                ? nodeSourceFromBinary(
                      process.env.BIFROSTWRITE_EMBEDDED_NODE_BIN_ARM64.trim(),
                  )
                : await downloadOfficialNodeSource("aarch64-apple-darwin");
        const x64Source = process.env.BIFROSTWRITE_EMBEDDED_NODE_BIN_X64?.trim()
            ? nodeSourceFromBinary(
                  process.env.BIFROSTWRITE_EMBEDDED_NODE_BIN_X64.trim(),
              )
    ...
    const configuredNodeBinary =
        process.env.BIFROSTWRITE_EMBEDDED_NODE_BIN?.trim();
...
function nodeBinaryFromSource(nodeSource, targetTriple) {
    const executable = executableNameForTarget("node", targetTriple);
    return nodeSource.kind === "portable-bin-directory"
        ? path.join(nodeSource.sourcePath, executable)
        : path.join(nodeSource.sourcePath, "bin", executable);
}

async function stageEmbeddedNodeRuntime(nodeSource, targetTriple) {
    ...
    await fs.copyFile(
        nodeBinaryFromSource(nodeSource, targetTriple),
        path.join(
            destinationBinDir,
            executableNameForTarget("node", targetTriple),
        ),
...
async function materializeStagingSource(filePath) {
    ...
    const tempDir = await fs.mkdtemp(
        path.join(os.tmpdir(), "bifrostwrite-tauri-stage-"),
    );
...
if (isUniversalMac) {
    stagingNativeBackendPath = await buildOrResolveUniversalRuntimeBinary({
        baseEnvKey: "BIFROSTWRITE_NATIVE_BACKEND_BUNDLE_BIN",
    ...
    if (
        !args.skipBuild &&
        !process.env.BIFROSTWRITE_NATIVE_BACKEND_BUNDLE_BIN?.trim() &&
        !process.env[
            targetSpecificEnvKey(
                "BIFROSTWRITE_NATIVE_BACKEND_BUNDLE_BIN",
                targetTriple,
            )
        ]?.trim()
    ) {
