...
const sidecarPath =
  process.env.BIFROSTWRITE_NATIVE_BACKEND_PATH?.trim() || defaultSidecar;
...
class SidecarClient {
  ...
  constructor(executablePath, appDataDir) {
    this.#child = spawn(executablePath, [], {
      ...
      env: {
        ...process.env,
        BIFROSTWRITE_APP_DATA_DIR: appDataDir,
        ...
        BIFROSTWRITE_AI_SECRET_STORE:
          process.env.BIFROSTWRITE_AI_SECRET_STORE?.trim() || "memory",
        BIFROSTWRITE_CUSTOM_ACP_TEST_SECRET: "must-not-reach-custom-runtime",
...
const sessionId = process.env.FAKE_ACP_SESSION_ID ?? "fake-native-acp-session";
...
createInterface({ input: process.stdin }).on("line", (line) => {
  ...
  if (message.method === "initialize") {
    ...
    result(message.id, {
      protocolVersion: 1,
      agentCapabilities,
      agentInfo: { name: "fake-native-acp", title: "Fake native desktop ACP", version: "0.0.0" }
    });
    return;
  }
...
async function main() {
  await fs.access(sidecarPath).catch(() => {
    throw new Error(
      `Missing sidecar binary at ${sidecarPath}. Run npm run tauri:sidecar:build first.`,
    );
  });
  ...
  try {
    ...
    assert(
      customCapture.env.BIFROSTWRITE_CUSTOM_ACP_TEST_SECRET === undefined &&
        customCapture.env.CUSTOM_ACP_INHERITED_ONLY === undefined &&
        customCapture.env.BIFROSTWRITE_AI_SECRET_STORE === undefined &&
    ...
    assert(
      ...
      "native desktop setup should accept a configured ACP runtime",
    );
    ...
    console.log("native desktop AI runtime sidecar smoke passed.");
  } finally {
    client.dispose();
